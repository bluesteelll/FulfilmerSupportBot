...
async def callback_fulfillment_start(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(text="Оставьте свой рабочий телефон, для того, чтобы наш менеджер уточнил все детали")
...
async def _finish(message: Message, data):
    ...
    if data['service'] == 'fulfillment':
        await message.answer_document(document=FSInputFile('data/Прайс.pdf.pdf'))
    print(data)
